...
import { ADMIN_CONSOLE, MONGODB, NGINX_PROXY } from '../fake_stack/common/constants';
...
export const generateEvent: GeneratorFunction = (config, schedule, index, timestamp) => {
  ...
  return [
    ...
    const message =
      event.namespace === MONGODB
        ? convertToMongoDBRawLog(event).message
        : event.namespace === NGINX_PROXY
        ? convertToNginxRawLog(event).message
        : JSON.stringify(
            omit(
              {
                ...event,
    ...
    set(finalDoc, 'host', get(event, 'host'));
